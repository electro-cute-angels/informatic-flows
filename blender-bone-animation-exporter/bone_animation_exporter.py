bl_info = {
    "name": "Animation to CSV Exporter",
    "author": "electro-cute-angels",
    "version": (1, 0),
    "blender": (2, 80, 0),
    "location": "File > Export > Bone Animation to CSV",
    "description": "Export bone animation data to CSV - part of 'informatic flows' series",
    "warning": "",
    "doc_url": "",
    ...
    if obj and obj.type == 'ARMATURE' and obj.pose:
        for i, bone in enumerate(obj.pose.bones):
            ...
            items.append((
                bone.name,
                bone.name,
                f"Export animation from bone: {bone.name}"
            ))
    
    items.insert(0, ("ALL", "All Bones", "Export animation from all bones (will create a wider CSV with multiple columns)"))
...
class BONE_OT_export_animation_csv(Operator, ExportHelper):
    ...
    bl_label = "Export Bone Animation to CSV"
    ...
    bone_to_export: EnumProperty(
        name="Select Bone",
        description="Choose which bone's animation to export",
    ...
    def draw(self, context):
        ...
        box = layout.box()
        box.label(text="✿˚* informatic flows / fbx to csv *˚☾")
    ...
    def execute(self, context):
        ...
        if not obj.animation_data or not obj.animation_data.action:
            self.report({'ERROR'}, f"No animation data found on armature {obj.name}")
            return {'CANCELLED'}
...
def menu_func_export(self, context):
    self.layout.operator(BONE_OT_export_animation_csv.bl_idname, text="Bone Animation to CSV (.csv)")
